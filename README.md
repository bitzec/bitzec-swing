...
2. Building from source code
   ...
   ```
   This takes a few seconds and when it finishes, it builds a JAR file `./build/jars/MyZcash.jar`. 
   You need to make this file executable:
   ```
   user@ubuntu:~/build-dir/zcash-swing-wallet$ chmod u+x ./build/jars/MyZcash.jar
   ```
   At this point the build process is finished the built GUI wallet program is the JAR 
   file `./build/jars/MyZcash.jar`. In addition the JAR file 
...
4. Running the installed Zcash GUI wallet
   ...
   Assuming you have already installed [Zcash](https://z.cash/) and the GUI Wallet `MyZcash.jar` in 
   directory `/home/user/zcash/src` one way to run it from command line is:
   ```
   user@ubuntu:~/build-dir/zcash-swing-wallet$ java -jar /home/user/zcash/src/MyZcash.jar
   ```
   If you are using Ubuntu (or similar ;) Linux you may instead just use the file manager and 
   right-click on the `MyZcash.jar` file and choose the option "Open with OpenJDK 8 Runtime". 
...
1. Limitation: if a messaging identity has been created (happens on first click on the messaging UI tab), then replacing the `wallet.dat` or changing the node configuration between mainnet and testnet will make the identity invalid. This will result in a wallet update error. To remove the error the directory `~/.MyZcash/messaging` may be manually renamed or deleted (when the wallet is stopped). **CAUTION: all messaging history will be lost in this case!**
...
1. Limitation: When using a natively compiled wallet version (e.g. `MyZcash.exe` for Windows) on a 
very high resolution monitor with a specifically configured DPI scaling (enlargement) factor to make GUI 
elements look larger, the GUI elements of the wallet actually do not scale as expected. To correct this on
Windows you need to right-click on `MyZcash.exe` and choose option:
```
